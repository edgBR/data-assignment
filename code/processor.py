...
def lag_processor(df_in, target_columns, lags=365, drop_nans=True) -> pd.DataFrame:
    """
    ...
    try:
        df_out = True
        logger.info(f'Successfully lagged {target_columns}')
...
def conversor(df_in, columns_in, columns_out, conversion_ratio) -> pd.DataFrame:
    ...
    try:
        ...
        logger.info(f'Successfully converted {columns_in} to EURO')
    except Exception as e:
        raise e
    return df_in